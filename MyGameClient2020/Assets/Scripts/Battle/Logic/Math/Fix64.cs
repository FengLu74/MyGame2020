...
namespace Battle.Logic.MMath
{
    ...
    public partial struct Fix64 : IEquatable<Fix64>, IComparable<Fix64>
    {
        ...
        public static Fix64 FractionStrToFix64(string s)
        {
            ...
            string[] strs = s.Split('/');
            Fix64 a = new Fix64(int.Parse(strs[0]));
            if (strs.Length == 1)
            {
                return a;
            }

            Fix64 b = new Fix64(int.Parse(strs[1]));
            return a / b;
        }

        public static Fix64 IntStrToFix64(string intStr, int scaleValue = 1)
        {
            return new Fix64(int.Parse(intStr)) / scaleValue;
        }
        ...
        {
            F = Clamp(F, -Fix64.One, Fix64.One);
            Fix64 angle = Asin(F) * 180 / Fix64.PI;
