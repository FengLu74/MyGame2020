...
namespace XLua
{
    ...
    public class LuaEnv : IDisposable
    {
...
#if UNITY_EDITOR
        // 检查lua调用次数
        public class CheckCallLuaTimes
        {
            public long m_nLuaCallBeginTime = 0;  // lua每一轮调用的起始时间
            public int m_nLuaCallTimes = 0;  // lua调用次数
        };
        Dictionary<string, CheckCallLuaTimes> dicFunctionCall = new Dictionary<string, CheckCallLuaTimes>();
#endif
        Dictionary<string, LuaBase> dict = new Dictionary<string, LuaBase>();
        /// <summary>
        /// 不进行缓存luaFunction
        /// </summary>
        ...
        public object[] CallLuaFunction(string name, params object[] args)
        {
            ...
            {
                var gen_push_types_setter = Global.Get<LuaFunction>(name);
                if(gen_push_types_setter!=null)
                {
                    results = gen_push_types_setter.Call(args);
                }
            }
//#if UNITY_EDITOR
//            if (dicFunctionCall.ContainsKey(name))
//            {
//                int nCallTimes = dicFunctionCall[name].m_nLuaCallTimes;
//                if (nCallTimes > 50)
//                {
//                    long nCurTime = DateTime.Now.Ticks;
//                    float fMinTime = ((float)(nCurTime - dicFunctionCall[name].m_nLuaCallBeginTime)) / 10000000.0f;
//                    if (fMinTime < 5.0f)
//                    {
//                        //Debugger.LogError("----LUA函数执行-name:" + name + "在" + fMinTime + "秒钟内执行次过多：" + dicFunctionCall[name].m_nLuaCallTimes);
//                    }
//                    dicFunctionCall[name].m_nLuaCallTimes = 0;
//                    dicFunctionCall[name].m_nLuaCallBeginTime = DateTime.Now.Ticks;
//                }

//                dicFunctionCall[name].m_nLuaCallTimes++;
//            }
//            else
//            {
//                CheckCallLuaTimes cCallTimes = new CheckCallLuaTimes();
//                cCallTimes.m_nLuaCallTimes = 1;
//                cCallTimes.m_nLuaCallBeginTime = DateTime.Now.Ticks;
//                dicFunctionCall.Add(name, cCallTimes);
//            }

//            if ((DateTime.Now.Ticks - nTime) * 0.0001f > 50)
//                //Debugger.LogError("----LUA函数执行超时-name:" + name + "运时时间：" + (DateTime.Now.Ticks - nTime) * 0.0001f);
